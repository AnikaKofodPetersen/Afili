...
if database_present == False:
	...
	if typestrain == False:
		command = "ls *fna | parallel -j " + cores.rstrip() + " 'makeblastdb -in ./{} -dbtype nucl -parse_seqids -out ../db/" + str(genus) + "/{}_DNA_DB' > /dev/null 2>&1"
		os.system(command)
		...
		command = "ls *fna | parallel -j " + cores.rstrip() + " 'makeblastdb -in {} -dbtype nucl -parse_seqids -out ../db/" + str(genus) + "T/{}T_DNA_DB' > /dev/null 2>&1"
		os.system(command)
		for fasta in os.listdir():
